...
from clip_sampling import build_metadata, frame_file, load_frames  # noqa: E402
from collate import ASSISTANT_MARKER, with_system  # noqa: E402,F401 (re-exported)

from sampling import DEFAULT_FRAME_SIZE, FPS, to_native_index  # noqa: E402
...
def clip_inputs(record: dict, frame_size: tuple[int, int] = DEFAULT_FRAME_SIZE,
    ...
    paths = [frame_file(directory, to_native_index(i, base_fps)) for i in idx5]
    video = load_frames(paths, frame_size)
...
def build_generation_inputs(processor, record: dict, system_prompt: str | None = None,
    ...
    video, meta = clip_inputs(record, frame_size, frames_root)
